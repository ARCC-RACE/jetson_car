...
if ! grep "export ROS_HOSTNAME=$(hostname).local" ~/.bashrc ; then
  ...
  echo "What is the hostname of the racecar you are using? (do not include suffix (i.e. *.local) and make sure that pinging racecar_hostname.local from controller works)\n Run \`hostname\` in a terminal on the jetson to see what the hostname is"
  read racecar_hostname
  echo $(racecar_hostname)
