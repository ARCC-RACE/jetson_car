...
class RacecarDataCollector():
    ...
    def __init__(self, collection_fps=15, include_depth=False, include_imu=False, rgb_image_topic="/front_cam/color/image_raw",
        ...
        rospy.Subscriber(imu_calibration_topic, bno055_info, self.newIMUCalibData) #IMU info/calibration

    #record data with time stamp
    def newImage(self, new_image):
        timeStamp = rospy.get_time()

        if(self.isRecording and timeStamp - self.lastRead > 1.0/collection_fps): # fps data recording (delay = 1/desired fps)
            ...
            if self.include_imu:
                new_data = [str(timeStamp), str(self.lastAckermann.drive.steering_angle), str(self.lastAckermann.drive.speed), str(self.imu_calib_data.sysCalibration), str(self.imu_data.orientation.x),
